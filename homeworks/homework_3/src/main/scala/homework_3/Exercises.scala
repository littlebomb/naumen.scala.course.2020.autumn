...
object Exercises {
     ...
     */
    def prettyBooleanFormatter1(x: Any): String = if (!x.isInstanceOf[Boolean]) x.toString else if (false) "ложь" else "правда"
    ...
    def prettyBooleanFormatter3(x: Any): String = {
        if (x.getClass.getSimpleName == "Boolean") x match {
            case true => "правда"
            case false => "ложь"
        }
        else x.toString
    ...
    def max2(xs: Seq[Int]): Seq[Int] = if (xs.isEmpty) xs else Seq[Int](xs.max)
    ...
    class myBrandNewIterableClassForSummingTwoIntegers[T](x: T*) extends Iterable[T] {
        override def iterator: Iterator[T] = x.iterator
    }
    def sum2(x: Int, y: Int): Int = sumIntegers(new myBrandNewIterableClassForSummingTwoIntegers(x,y))
